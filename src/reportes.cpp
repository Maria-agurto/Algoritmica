...
// participantes.h. Las inscripciones se leen igual que en
// inscripcionwindow.cpp (Diego), replicando el formato
// binario de tamano-prefijado de Darren.
//====================================================

static void leerStringR(ifstream &archivo, string &texto){
	size_t tamano=0;
...
static bool leerInscripcionR(ifstream &archivo, Inscripcion &insc){
	...
	leerStringR(archivo, insc.codigoAlumno);
	leerStringR(archivo, insc.fechaRegistro);
...
static vector<Inscripcion> cargarInscripciones() {
	vector<Inscripcion> lista;
	ifstream archivo(AINSCRIPCIONES, ios::binary);
	if(!archivo.is_open()) return lista;

	Inscripcion actual;
	while(leerInscripcionR(archivo, actual)){
